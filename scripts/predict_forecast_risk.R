...
cur <- prepare_prediction(read_csv(forecast_env_path, show_col_types = FALSE), train, depth_path)
fit <- fit_primary_gam(train)
out <- score_relative_risk(fit, train, cur) %>% arrange(offset_days, desc(core_percentile))
write_csv(out, forecast_out_path)
write_csv(out %>% filter(offset_days == min(offset_days, na.rm = TRUE)), latest_out_path)
...
cat("Saved:", forecast_out_path, "and", latest_out_path, "\n")
