...
forecast_env_path <- file.path(APP_ROOT, "outputs", "forecast_env.csv")
forecast_out_path <- file.path(APP_ROOT, "outputs", "forecast_risk.csv")
latest_out_path <- file.path(APP_ROOT, "outputs", "latest_risk.csv")
...
kuroshio_context_from_distance <- function(dist_km) {
  exp(-0.5 * ((dist_km - 265.6) / 180)^2)
}

if (!file.exists(master_path)) stop(paste("Missing:", master_path))
if (!file.exists(forecast_env_path)) stop(paste("Missing:", forecast_env_path))

df <- read_csv(master_path, show_col_types = FALSE)
cur <- read_csv(forecast_env_path, show_col_types = FALSE)
...
if (!("net_name" %in% names(cur))) {
  cur$net_name <- paste0("NetID ", cur$NetID)
}

if (!("net_label" %in% names(cur))) {
  cur$net_label <- paste0(cur$net_name, "（NetID ", cur$NetID, "）")
}
...
if (requireNamespace("maxnet", quietly = TRUE)) {
  ...
  mx_x <- as.data.frame(df_mx %>% select(Jday_sin, Jday_cos, SST, depth_m))
  mx_p <- df_mx$presence
  ...
  cur$maxent_suitability <- as.numeric(
    predict(
      mx_fit,
      newdata = as.data.frame(cur_mx %>% select(Jday_sin, Jday_cos, SST, depth_m)),
      type = "cloglog"
    )
  )
...
if ("kuroshio_dist_km" %in% names(cur)) {
  cur$kuroshio_dist_km <- as.numeric(cur$kuroshio_dist_km)
  cur$kuroshio_context <- kuroshio_context_from_distance(cur$kuroshio_dist_km)
  cur$kuroshio_context[is.na(cur$kuroshio_context)] <- 0.5
  has_kuroshio <- any(!is.na(cur$kuroshio_dist_km))
} else {
  cur$kuroshio_dist_km <- NA_real_
  cur$kuroshio_context <- NA_real_
  has_kuroshio <- FALSE
}
...
if (has_kuroshio) {
  cur <- cur %>%
    mutate(
      integrated_risk = 0.75 * core_risk +
        0.15 * kuroshio_context +
        0.10 * upwelling_context,
      integrated_formula = "0.75 core + 0.15 kuroshio + 0.10 upwelling"
    )
} else {
  cur <- cur %>%
    mutate(
      ...
      integrated_formula = "0.85 core + 0.15 upwelling; kuroshio not implemented"
    )
}
...
cur <- cur %>%
  ...
  mutate(
    integrated_percentile_today = rank(integrated_risk, ties.method = "average") / n(),
...
cur <- cur %>%
  mutate(
    ...
    note = ifelse(
      has_kuroshio,
      "Forecast risk is experimental. SST is based on historical seasonal climatology. Core GAM risk should be treated as the main risk index. Kuroshio context is included.",
      "Forecast risk is experimental. SST is based on historical seasonal climatology. Core GAM risk should be treated as the main risk index."
    )
  ) %>%
  arrange(offset_days, desc(core_risk))

dir.create(dirname(forecast_out_path), recursive = TRUE, showWarnings = FALSE)

write_csv(cur, forecast_out_path)

latest <- cur %>%
  filter(offset_days == 0) %>%
  arrange(desc(core_risk))

write_csv(latest, latest_out_path)
...
metrics <- tibble(
  ...
  maxent_source = maxent_source,
  has_kuroshio = has_kuroshio,
  forecast_offsets = paste(sort(unique(cur$offset_days)), collapse = ","),
  integrated_formula = ifelse(
    has_kuroshio,
    "0.75 core + 0.15 kuroshio + 0.10 upwelling",
    "0.85 core + 0.15 upwelling; kuroshio not implemented"
  ),
...
cat("Saved:", forecast_out_path, "\n")
cat("Saved:", latest_out_path, "\n")

print(
  cur %>%
    select(offset_days, target_date, net_name, NetID, Jday, SST, depth_m, core_risk, kuroshio_dist_km, kuroshio_context, integrated_risk, integrated_risk_class) %>%
    group_by(offset_days) %>%
    slice_max(order_by = integrated_risk, n = 5) %>%
    ungroup()
)
